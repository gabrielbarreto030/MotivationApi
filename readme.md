...
Day 74: Goal progress timeline (GET /goals/{id}/timeline: returns a chronological list of completed steps for a specific goal showing how it progressed over time; GoalTimelineEntry record with StepId, StepTitle, CompletedAt; GoalTimelineResponse record with GoalId, GoalTitle, TotalSteps (all steps), CompletedSteps (only completed), and Entries (IReadOnlyList<GoalTimelineEntry> ordered by CompletedAt asc); pending steps are excluded from Entries; returns 404 if goal not found or belongs to another user; IGoalTimelineService/GoalTimelineService aggregates steps from IStepRepository filtering to completed ones and ordering by CompletedAt asc; registered in DI; endpoint added to GoalsController; GoalTimelineTests.cs covers goal not found returns null, goal belonging to other user returns null, no steps returns empty entries, only pending steps returns empty entries, single completed step included with correct fields, mixed steps TotalSteps counts all but Entries only completed, multiple steps ordered by CompletedAt asc, GoalId/GoalTitle correctly mapped, data isolation between users, and combined full scenario) - OK
Day 75: Most productive weekday (GET /users/weekday-stats: returns step completion counts grouped by day of the week (Monday-Sunday) across all time for the authenticated user; WeekdayStatsResponse DTO with TotalStepsCompleted, MostProductiveDay (nullable string - day name with most completions, null if no steps), and Entries (IReadOnlyList<WeekdayEntry> always 7 entries ordered Monday-Sunday with DayName and StepsCompleted); WeekdayEntry record; IWeekdayStatsService/WeekdayStatsService aggregates step CompletedAt dates from all user goals grouping by DayOfWeek UTC using Monday-based index (Monday=0...Sunday=6); registered in DI; endpoint added to UsersController; WeekdayStatsTests.cs covers no goals returns all zeros and null most productive day, only pending steps returns all zeros, always 7 entries ordered Monday–Sunday, single Monday step counted under Monday, single Sunday step counted under Sunday, multiple steps same day aggregated, most productive day correctly identified, steps across multiple goals combined, data isolation between users, mixed completed/pending only completed counted, TotalStepsCompleted equals sum of all entries, and combined full scenario) - OK
