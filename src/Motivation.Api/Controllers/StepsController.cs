...
namespace Motivation.Api.Controllers
{
    ...
    public class StepsController : ControllerBase
    {
        ...
        /// <summary>
        /// Retorna estatísticas agregadas dos passos de uma meta do usuário autenticado.
        /// </summary>
        /// <param name="goalId">Id da meta.</param>
        /// <returns>Estatísticas: total, concluídos, pendentes, vencidos, percentual de conclusão, breakdown por prioridade e por tag.</returns>
        ...
        public async Task<IActionResult> GetStats(Guid goalId)
        {
            ...
            try
            {
                var result = await _stepService.GetStatsAsync(goalId, userId.Value);
                _logger.LogInformation("Stats retrieved for goal {GoalId} by user {UserId}", goalId, userId.Value);
                return Ok(result);
            }
            catch (ArgumentException ex)
            {
                _logger.LogWarning("Bad request getting step stats for goal {GoalId}: {Message}", goalId, ex.Message);
                return BadRequest(new { message = ex.Message });
            }
            catch (UnauthorizedAccessException ex)
            {
                _logger.LogWarning("Unauthorized step stats attempt on goal {GoalId} by user {UserId}: {Message}", goalId, userId.Value, ex.Message);
                return Forbid();
            }
        }
        ...
        /// <param name="search">Filtrar por palavra-chave: busca parcial, insensível a maiúsculas, em Title e Notes (opcional).</param>
            ...
            [FromQuery] string? search = null)
        {
            ...
            try
            {
                var filterRequest = new StepFilterRequest(page, pageSize, isCompleted, sortBy, sortOrder, priority, tag, search);
                var result = await _stepService.ListByGoalFilteredAsync(goalId, userId.Value, filterRequest);
                _logger.LogInformation(
                    "Listed {Count}/{Total} steps for goal {GoalId} by user {UserId} (isCompleted: {IsCompleted}, priority: {Priority}, tag: {Tag}, search: {Search}, sortBy: {SortBy}, sortOrder: {SortOrder})",
                    result.Items.Count, result.TotalCount, goalId, userId.Value,
                    isCompleted?.ToString() ?? "all", priority?.ToString() ?? "all", tag ?? "all", search ?? "all", sortBy ?? "order", sortOrder ?? "asc");
                return Ok(result);
            }
            catch (ArgumentException ex)
            {
